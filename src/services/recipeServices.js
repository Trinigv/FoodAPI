...
const getAPI = async () => {
	...
	await respuesta.data.results?.map((el) => {
		Recipe.bulkCreate({
			id: el.id,
			title: el.title,
			healthScore: el.healthScore,
...
const filterFunction = async (req, res, next) => {
	...
	try {
		if (title) {
			var recipeFiltered = await recipes.filter((el) =>
				el.title.toLowerCase().includes(title.toLowerCase())
			);
			recipeFiltered.length
				? res.send(recipeFiltered)
				: res.send('No se ha encontrado la receta');
		} else {
			res.send(recipes);
		}
	} catch (e) {
	...
	try {
		if (id) {
			...
			recipeId.length
				...
				: res
						.status(404)
						.send(`No se ha encontrado receta con id ${id}`);
		}
	} catch (e) {
		next(e);
	}
...
const createRecipe = async (req, res, next) => {
	const { title, summary, healthScore, instructions, image, diets } =
		req.body;
	if (!title || !summary)
		return res.status(404).json({ err: 'Faltan datos obligatorios' });
	try {
